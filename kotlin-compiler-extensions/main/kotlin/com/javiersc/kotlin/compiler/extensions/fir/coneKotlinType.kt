...
public inline fun isSubtypeOf(
    subType: ConeKotlinType,
    superType: ConeKotlinType,
): Boolean = AbstractTypeChecker.isSubtypeOf(session.typeContext, subType, superType)

context(session: FirSession)
public inline fun <reified T> coneKotlinType(
    vararg typeArguments: ConeTypeProjection
): ConeKotlinType = coneKotlinType<T>(typeArguments.toList().toTypedArray())

@JvmName("coneKotlinTypeWithTypeArgumentsArray")
context(session: FirSession)
public inline fun <reified T> coneKotlinType(
    typeArguments: Array<ConeTypeProjection> = emptyArray()
): ConeKotlinType = classId<T>().createConeType(session, typeArguments)
